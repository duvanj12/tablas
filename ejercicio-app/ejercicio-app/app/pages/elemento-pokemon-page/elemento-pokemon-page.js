...
class elementoPokemonPage extends intl(CellsPage) {
  static get is() {
    return 'elemento-pokemon-page';
  }
    ...
    return html`
      ...
      <cells-template-paper-drawer-panel page-title="${this.t(this.i18nKeys.loginTitle)}" state="active">
        <div slot="app__header">
          <h1>Elemento del Pokémon</h1>
        </div>

        <div slot="app__main">
          <p><strong>Nombre:</strong> ${this.pokemon?.name || 'N/A'}</p>
          <p><strong>Elemento:</strong> ${this.pokemon?.types?.[0]?.type?.name || 'N/A'}</p>
...
window.customElements.define(elementoPokemonPage.is, elementoPokemonPage);